...
class InvoiceGateway extends BaseGateway
{
    ...
    {
        $response = $this->twikey->request('POST', "/creditor/invoice", ['form_params' => $data, 'headers' => $optionalHeaders]);
    ...
    {
        $response = $this->twikey->request('GET', sprintf("/creditor/invoice/%s", $id), []);
     ...
     * @throws ClientExceptionInterface
     */
    public function feed(InvoiceCallback $callback)
    {
        $count = 0;
        do {
            $response = $this->twikey->request('GET', "/creditor/invoice", []);
